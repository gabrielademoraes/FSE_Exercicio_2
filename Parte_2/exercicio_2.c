 ...
 *
 */
void print_sensor_data(struct bme280_data *comp_data, float* temperaturas, float* pressoes, float* umidades, int contador);
 ...
 *
 */
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev, FILE* mediaDadosDe10em10segundos);
...
int main(int argc, char* argv[]){
    ...
    FILE* mediaDadosDe10em10segundos;
    ...
    rslt = stream_sensor_data_forced_mode(&dispositivo, mediaDadosDe10em10segundos);
    if (rslt != BME280_OK){
...
void print_sensor_data(struct bme280_data *comp_data, float* temperaturas, float* pressoes, float* umidades, int contador){
    ...
    pressoes[contador] = press;
    umidades[contador] = hum;
}
...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev, FILE* mediaDadosDe10em10segundos){
    ...
    req_delay = 1000000; //1 segundo

    
    int contador = 0;
    float temperaturas[10], pressoes[10], umidades[10];
    float mediaTemperaturas, mediaPressoes, mediaUmidades;
    ...
    while (1){
        ...
        if(contador == 10){
            // for(int i = 0; i < 10; i++){
            //     printf("\nT: %0.2lf\t P: %0.2lf\t U: %0.2lf", temperaturas[i], pressoes[i], umidades[i]);
            // }
            mediaTemperaturas = calculaMedia(temperaturas, 10);
            mediaPressoes = calculaMedia(pressoes, 10);
            mediaUmidades = calculaMedia(umidades, 10);
            printf("\nMedias: T:%0.2lf °C\t P:%0.2lf hPa\t U:%0.2lf %%", mediaTemperaturas, mediaPressoes, mediaUmidades);
            mediaDadosDe10em10segundos = fopen("mediaDados.csv","a+");
            if(!mediaDadosDe10em10segundos){
                printf("\nFalha na abertura do arquivo");
                exit(1);
            }
            time_t clk = time(NULL);
            fprintf(mediaDadosDe10em10segundos,"%sTemperatura: %0.2lf deg C\nPressão: %0.2lf hPa\nUmidade: %0.2lf%%\n\n", ctime(&clk), mediaTemperaturas, mediaPressoes, mediaUmidades);
            fclose(mediaDadosDe10em10segundos);
            contador = 0;
        ...
        print_sensor_data(&comp_data, temperaturas, pressoes, umidades, contador);
        contador += 1;
        printf("\n%d", contador);
    }
